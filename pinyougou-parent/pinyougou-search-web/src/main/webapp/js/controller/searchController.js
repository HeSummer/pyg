app.controller('searchController',function($scope,$location,searchService){
    ...
    $scope.search=function () {
        $scope.searchMap.pageNo=parseInt($scope.searchMap.pageNo) ;//修改 search 方法, 在执行查询前，转换为 int 类型，否则提交到后端有可能变成字符串
   ...
   $scope.addSearchItem=function (key,value) {
       if(key=='category' || key =='brand'|| key=='price'){//如果点击的分类或者是品牌
    ...
    buildPageLabel=function () {
        ...
        if($scope.resultMap.totalPages > 5){//如果总页数大于5页,显示部分页码
                if($scope.searchMap.pageNo <= 3){ //如果当前页小于等于3
                    lastPage=5; //截止页为5
                    $scope.firstDot=false;//前面点关闭
                }else if($scope.searchMap.pageNo > lastPage-2){//如果当前页大于等于最大页码-2

                    firstPage= maxPageNo-4;  //后 5 页
                    $scope.lastDot=false;//后面点关闭
                }else {  //显示当前页为中心的 5 页
